...
/** LiveData events class */
class MutableLiveEvent<T : Event<Any>> : MutableLiveData<T>() {

    internal class PendingObserver<T : Event<Any>>(val wrappedObserver: Observer<in T>) :
        Observer<T> {
    ...
    private val observers = ArraySet<PendingObserver<in T>>()
    ...
    override fun removeObserver(observer: Observer<in T>) {
        ...
        when (observer) {
            is PendingObserver -> {
                ...
                val pendingObserver = observers.firstOrNull {
                    it.wrappedObserver == observer
                }
